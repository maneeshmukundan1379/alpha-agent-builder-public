...
/**
 * Empty string => same origin as the page (Vite proxies /api → FastAPI). Required for tunnel URLs
 * (ngrok, cloudflared). Optional: set VITE_API_BASE in frontend/.env for a direct API URL.
 */
const API_BASE =
...
const TOKEN_SESSION_KEY = "alpha-agent-builder-public-token-session";
/** Only set when the user checks "Keep me signed in". */
const TOKEN_PERSISTENT_KEY = "alpha-agent-builder-public-token-persistent";
/** Pre–split-behavior key (always localStorage). Removed on load so old tokens cannot skip login. */
const LEGACY_TOKEN_KEY = "alpha-agent-builder-public-token";
...
export async function fetchUserEnv(): Promise<UserEnvResponse> {
  return request<UserEnvResponse>("/api/user-env");
}

export async function saveUserEnv(content: string): Promise<UserEnvResponse> {
  return request<UserEnvResponse>("/api/user-env", {
    method: "PUT",
...
export async function checkInAgent(
  agent_id: string,
  payload: CheckInAgentRequestBody,
): Promise<CheckInAgentResponse> {
