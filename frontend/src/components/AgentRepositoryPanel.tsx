...
export function AgentRepositoryPanel({ onBanner }: AgentRepositoryPanelProps) {
  ...
  const [checkInRepoUrl, setCheckInRepoUrl] = useState("");
  ...
  const handleCheckIn = async () => {
    ...
    const repo_url = checkInRepoUrl.trim();
    if (!repo_url) {
      onBanner({ kind: "error", text: "Enter the GitHub repository URL below before checking in." });
      return;
    }
    if (
      !window.confirm(
        "Push this agent to the GitHub URL below using GITHUB_TOKEN from your saved Environment file? (Add it under Create/Edit Environment file if you have not yet.)",
      )
    ) {
      return;
    }
    ...
    onBanner(null);
    try {
      const result = await checkInAgent(selectedAgentId, { repo_url });
      await refreshAgents();
  ...
  if (agents.length === 0) {
    return (
      <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 text-center text-sm text-slate-400 shadow-xl shadow-slate-950/30">
        Generate an agent above to manage its repository: Run, Edit/fix agent, Delete, then tree, Git check-in, and
        refresh.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-violet-950/20">
      ...
      <p className="mt-1 text-sm text-slate-300">
        ...
        terminal-style console. <span className="text-amber-200">Edit/fix agent</span> sits between Run and Delete to
        change or fix code from one text field. Delete removes the agent. Check in and refresh apply to the file tree.
      </p>
      ...
      <div className="mt-4 space-y-4 border-t border-white/10 pt-4">
        <div className="grid max-w-xl gap-3">
          <label className="space-y-2">
            <span className="text-sm font-medium text-slate-200">GitHub repository URL (this check-in only)</span>
            <input
              autoComplete="off"
              className="w-full rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:border-violet-400/60 focus:outline-none"
              onChange={(e) => setCheckInRepoUrl(e.target.value)}
              placeholder="https://github.com/your-org/your-repo.git"
              type="url"
              value={checkInRepoUrl}
            />
          </label>
          <p className="text-xs text-slate-500">
            Authentication uses <code className="text-slate-400">GITHUB_TOKEN</code> from your saved{" "}
            <span className="text-slate-400">Create/Edit Environment file</span> only—not this page.
          </p>
        </div>
