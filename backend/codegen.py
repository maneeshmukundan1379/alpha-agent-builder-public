...
def _declared_env_var_names(config: AgentConfigRequest, user_env_var_names: frozenset[str]) -> list[str]:
    provider = get_provider(config.provider_id)
    names = set(provider["secret_names"])
    names.update(user_env_var_names)
...
def try_generate_logic_py(
    ...
    *,
    settings: dict[str, str],
    user_env_var_names: frozenset[str],
...
9. The payload includes `declared_env_var_names`: every name the user may have in `.env` (their global Env file keys, provider defaults, plus per-agent custom keys). Use `os.getenv` for any third-party or integration APIs the instructions require (e.g. news, maps, databases) when those names appear in `declared_env_var_names` or are implied by the task.
