...
from .auth_store import get_user_process_env
...
def run_agent(user_id: int, agent_id: str, prompt: str) -> tuple[RunRecord, str | None]:
    global _active_run_id
    ...
    if metadata.frontend_type in ("gradio", "react"):
        run, url = _start_local_ui_server(user_id, metadata)
        return run, url
    ...
    with _run_lock:
        ...
        env = {**dict(os.environ), **get_user_process_env(user_id), "ALPHA_AGENT_MODEL": metadata.model}
        process = subprocess.Popen(
            command,
            cwd=agent_dir,
