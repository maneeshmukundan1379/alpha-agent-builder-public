...
def render_project_files(
    *,
    ...
    files = {
        ...
        "agent_config.json": _render_agent_config(
            config, provider, requirements, secret_names, user_env_var_names
        ),
...
def _render_agent_config(
    ...
    declared.update(user_env_var_names)
...
def _render_readme(config: AgentConfigRequest, provider: dict) -> str:
    ...
    return dedent(
        ...
        At generate time, Alpha Agent Builder copies your saved global Env file into a local `.env`. That file is
        gitignored. If you run the project manually, either keep `.env` in
        the project root or export the same variables in your shell.
        When using the builder Run tab, those same variables are injected into the process environment.
        {run_section.strip()}
