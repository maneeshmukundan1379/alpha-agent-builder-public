...
from .auth_store import get_user_env_file_text
...
GENERATED_AGENTS_DIR = app_data_dir() / "generated_agents"
...
def generate_agent_project(
    ...
    *,
    ...
    if len(config.agent_name.strip()) < 3:
        raise ValueError("Agent name must be at least 3 characters.")
    if len(config.description.strip()) < 10 or len(config.instructions.strip()) < 10:
        raise ValueError(
            'Add at least 10 characters under "What should the agent do?" before generating.'
        )
    ...
    user_env_text = get_user_env_file_text(int(user["id"]))
    user_env_keys = frozenset(parse_dotenv_content(user_env_text).keys())
    ...
    rendered_files = render_project_files(
        ...
        user_env_var_names=sorted(user_env_keys),
    )

    generation_source: Literal["llm", "template"] = "template"
    logic_llm, extra_reqs, _gen_note = try_generate_logic_py(
        config,
        settings=settings,
        user_env_var_names=user_env_keys,
    )
    ...
    repo_url = (config.github_repo_url or "").strip()
...
def checkin_generated_agent(
    ...
    *,
    ...
    clean_token = (github_token or "").strip()
    if not clean_url:
        raise ValueError("Enter the GitHub repository URL for this check-in.")
    if not clean_token:
        raise ValueError("Enter a GitHub personal access token for this check-in.")
