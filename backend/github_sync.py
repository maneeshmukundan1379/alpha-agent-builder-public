...
REPO_CACHE_DIR = app_data_dir() / "repo_workdirs"
...
def _push_error_with_hint(message: str) -> str:
    ...
    if "403" in text or ("permission" in lower and "denied" in lower) or "access denied" in lower:
        text += (
            ...
            "Then paste the new token into Alpha Agent Builder → Env → GITHUB_TOKEN=... and save."
        )
    return text
...
def _git_auth_args(github_token: str) -> list[str]:
    ...
    if not token:
        raise ValueError("Add GITHUB_TOKEN to your Env file before using a GitHub repo.")
