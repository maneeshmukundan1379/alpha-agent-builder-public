# Alpha Agent Builder (public web copy)

This tree is **`alpha_agent_builder_public`**: deploy **independently on any server** with **`docker compose`** (see **`WEB_PUBLIC_README.md`**). After deployment, nothing depends on your laptop. Optional local dev: **`./start_public_web.sh`**. The original `alpha_agent_builder` folder is separate.

---

`Alpha Agent Builder` lets a user:
